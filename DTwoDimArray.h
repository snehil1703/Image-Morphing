...
class _DTwoDimArray
{
 public:
  ...
  void addzeros(int max)
  {
      ...
      x = rtemp+(_rows - rtemp)/2;
      y = ctemp+(_cols - ctemp)/2;
      for(int i=(_rows - rtemp)/2, a=0; i<x; i++, a++)
      {
        for(int j=(_cols - ctemp)/2, b=0; j<y; j++, b++)
        {
          data_area[(_cols * i)+j] = tempdata[a][b];
        }
      }
      tempdata = NULL;
  }
